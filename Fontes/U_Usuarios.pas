...
type
  TF_usuarios = class(TForm)
    ...
    SQL_gravarTelaACESSO_ID: TIntegerField;
    SQL_gravarTelaACESSO_USER: TIntegerField;
    SQL_gravarTelaACESSO_TELA: TIntegerField;
...
begin
  ...
  begin
   ...
   {SQL_gravarTela.Active :=True;
   SQL_gravarTela.Insert;
   SQL_gravarTelaACESSO_ID.AsInteger :=DM.sql_Gen_acessoID.Value;}

   with SQL_gravarTela  do
   begin
   Close;
   SQL.Clear;
   SQL.Add('insert into acesso (acesso_id, acesso_user, acesso_tela)');
   SQL.Add('values (:id, :user, :tela)');
   ParamByName('id').Value := DM.sql_Gen_acessoID.Value;
   ParamByName('user').Value := SQL_listarUsuariosID_USUARIO.Value;
   ParamByName('tela').Value := SQL_listarTelasforms_id.Value;
   ExecSQL;
   end;
